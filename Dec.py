...
with open('dice.json', 'r') as myfile:
      data=myfile.read()
# parse file
obj = json.loads(data)
...
. #######::. #######::. #######:: ########::'####:. ######:: ########:
:.......::::.......::::.......:::........:::....:::......:::........:: 
{biru}[Premium Script 1.01]🎲
{biru}Server Status :{hijau2} [ONLINE]                                                                                                                                                                                                                                                                                           
{hijau}============================================================
{red}[ {biru}CREATED BY 𝓜𝓸𝓷𝓷𝓪𝓵𝓲𝓼𝓪{red}]
{hijau}Support By {red}: {biru}Members 999Doge Discussion, @monnzila
{hijau}[Thanks For Support This Script🙏] 
{hijau}Donation {red}: {hijau}D61G6V8CEQysrXMSxbmGy1cSwaT7GiA8VW(Doge)
{hijau}Donation {red}: {hijau}MFvj2yQ8LW6J7a89UQps9RTi6uqVQsZkBv(Ltc)
{hijau}============================================================
{red}[{biru} SCRIPT 999DICE ALL CURRENCY  {red}]
{red2}Disclamer :
[ID]Semua Betset Memiliki Resiko Tidak ada Betset Yang Aman! 
[ENG]:All Betset has Risk No One Betset Is Safe! 
{hijau}============================================================\n""")
...
if pilihan == 'doge':
	cry = "Doge"
	target = obj["Withdraw"]["Wallet_doge"]
	wlt = "Wallet_doge"
if pilihan == 'ltc':
	cry = "LTC"
	target = obj["Withdraw"]["Wallet_ltc"]
if pilihan == 'btc':
	cry = "BTC"
	target = obj["Withdraw"]["Wallet_btc"]
if pilihan == 'eth':
	cry = "ETH"
	target = obj["Withdraw"]["Wallet_eth"]
...
def dice(ws,ls):
   ...
   try:
     ...
     print (hijau+"\n\nStarting Balance",res+str(rev(str(star_bal)))+hijau+" "+cry)
     n = 0
     burst = False
     ...
     #try:
        ...
        if blance > ws:
           if _lowercase(obj["Interface"]["Random"]["Auto"]) == "on":
              print ('\x1b[0;30;42m'+" "+str(rolebet)+" "+'\x1b[0m'+" "+ij+str(rev(str(amount))),"Profit",ij+rev(str(profw)),"Balance",ij+str(rev(str(blance)))+hijau+" "+cry+rs+" "+chance)
              print ('\x1b[1;32;100m'+" "*18+"STOP TARGET BALANCE"+" "*18+'\x1b[0m')
           else:
              print ('\x1b[0;30;42m'+" "+str(rolebet)+" "+'\x1b[0m'+" "+ij+str(rev(str(amount))),"Profit",ij+rev(str(profw)),"Balance",kn+str(rev(str(blance)))+hijau+" "+cry+rs)
              print ('\x1b[1;32;100m'+" "*17+"STOP TARGET BALANCE"+" "*17+'\x1b[0m')
           sys.exit()

        if blance < ls:
           print ('\x1b[0;37;41m'+" "+str(rolebet)+" "+'\x1b[0m'+rd+"-"+str(rev(str(amount))),"Lose  "+rd+"-"+rev(str(profl)),"Balance",kn+str(rev(str(blance)))+hijau+" "+cry+rs)
        ...
        if jsn["PayOut"] is not a:
           ...
           if prof > 0:
              if _lowercase(obj["Interface"]["Random"]["Auto"]) == "on":
                 print ('\x1b[0;30;42m'+" "+str(rolebet)+" "+'\x1b[0m'+" "+ij+str(rev(str(amount))),"Profit",ij+rev(str(profw)),"Balance",kn+str(rev(str(bal)))+hijau+" "+cry+rs+" "+chance)
              else:
                 print ('\x1b[0;30;42m'+" "+str(rolebet)+" "+'\x1b[0m'+" "+ij+str(rev(str(amount))),"Profit",ij+rev(str(profw)),"Balance",kn+str(rev(str(bal)))+hijau+" "+cry+rs)
           else:
             print ('\x1b[0;30;42m'+" "+str(rolebet)+" "+'\x1b[0m'+" "+ij+str(rev(str(amount))),"Lose  "+rd+"-"+rev(str(profl)),"Balance",kn+str(rev(str(bal)))+hijau+" "+cry)
        else:
           no_win = 0
           no_lose +=1
           i = 0
           ...
           bal = blance
           if prof > 0:
              print ('\x1b[0;37;41m'+" "+str(rolebet)+" "+'\x1b[0m'+rd+"-"+str(rev(str(amount))),"Profit",ij+rev(str(profw)),"Balance",kn+str(rev(str(bal)))+hijau+" "+cry+rs)
           else:
              print ('\x1b[0;37;41m'+" "+str(rolebet)+" "+'\x1b[0m'+rd+"-"+str(rev(str(amount))),"Lose  "+rd+"-"+rev(str(profl)),"Balance",kn+str(rev(str(bal)))+hijau+" "+cry+rs)
        total_roll+=1
        ...
        if _lowercase(obj["Withdraw"]["Auto"]) == "on":
           ...
           if blance > triger:
              ...
              if wd.status_code == 200:
                 if _lowercase(obj["Interface"]["Random"]["Auto"]) == "on":
                    print (bg_ij+" "*8+"Success Withdraw "+str(rev(str(send)))+status+" "*8+bg_end)
                 else:
                    print (bg_ij+" "*3+"Success Withdraw "+str(rev(str(send)))+status+" "*4+bg_end)
              else:
                 if _lowercase(obj["Interface"]["Random"]["Auto"]) == "on":
        ...
        if no_lose > total_lose:
           ...
           stats_rolebet_win = False
           total_lose +=1
        sys.stdout.write(" "+bg_ab2+" Seed "+seed_on_off+rs+kn+" "+bg_end+" "+bg_ij+" WS "+str(total_win)+" "+bg_end+" "+bg_rd+" LS "+str(total_lose)+" "+bg_end+" "+chance+" Roll : "+str(total_roll)+"\r")


   except:
     print ("")
     sys.exit()
r = c.get(url,headers=ua,data={"a": "Login","Key": "30b5de9739ad44f18651bd9d7adc9ca3","Username": obj["Account"]["Username"],"Password": obj["Account"]["Password"],"Totp": ""})
js = json.loads(r.text)
try:
  print (hijau+"Balance "+abu2+": "+res+str(float(js[cry]["Balance"])/(10 ** 8))+hijau+" "+cry)
except:
